...
Dotplot(others,scvi,'p1',c('SCMAP','SCANVI'),'temp.pdf')
Dotplot <- function(others,scvi,ann,methods,plotname){
	...
	if('SCMAP' %in% method){df$SCMAP = res[rownames(res)=='scmap',]}
	if('CCA' %in% method){df$CCA = res[rownames(res)=='readSeurat',]}
	if('SCANVI' %in% method){df$SCANVI = res[rownames(res)=='scanvi',]}
	...
	scale_x_continuous(breaks=vae$x,labels=celltypes) +
...
# rownames(res) = c(others[,1],scvi[,1],'scmap')
