...
class TestComputeKlWeight:
    ...
    def test_compute_kl_weight_linear_annealing(
        self, current, n_warm_up, min_kl_weight, max_kl_weight, expected
    ):
    ...
    def test_compute_kl_weight_no_annealing(self, max_kl_weight):
        assert _compute_kl_weight(1, 1, None, None, max_kl_weight, 0.0) == max_kl_weight

    def test_compute_kl_weight_min_greater_max(self):
    ...
    def test_compute_kl_precedence(
        self, epoch, step, n_epochs_kl_warmup, n_steps_kl_warmup, expected
    ):
        kl_weight = _compute_kl_weight(
