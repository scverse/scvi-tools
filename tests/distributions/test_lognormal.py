...
def test_zero_inflated_lognormal_distribution():
    ...
    dist = ZeroInflatedLogNormal(mu=mu, scale=scale, zi_logits=zi_logits, validate_args=True)
    ...
    lognormal_mean = torch.exp(mu + 0.5 * scale**2)
    ...
    # Test with normal_mu (normalized expression) parameter
    normal_mu = torch.tensor([0.5, 0.5])
    dist_with_norm_mu = ZeroInflatedLogNormal(
        mu=mu, scale=scale, zi_logits=zi_logits, normal_mu=normal_mu, validate_args=True
    )
    dist_with_norm_mu.__repr__()
    assert torch.allclose(dist_with_norm_mu.get_normalized("mu"), mu)
    assert torch.allclose(dist_with_norm_mu.get_normalized("scale"), normal_mu)

    # Test different batch sizes
    size = (50, 3)
    mu_batch = torch.randn(size)
    scale_batch = torch.rand(size) + 0.1
    zi_logits_batch = torch.randn(size)
    x_batch = torch.rand(size) + 0.1

    dist_batch = ZeroInflatedLogNormal(
        mu=mu_batch, scale=scale_batch, zi_logits=zi_logits_batch, validate_args=True
    )
...
def test_sampling_statistics():
    ...
    dist_ziln = ZeroInflatedLogNormal(mu=mu, scale=sigma, zi_logits=zi_logits)
