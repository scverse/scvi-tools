...
def test_synthetic_1():
    ...
    run_benchmarks(synthetic_dataset, n_epochs=1, use_batches=True, model=VAE, use_cuda=use_cuda)
    run_benchmarks_classification(synthetic_dataset, n_epochs=1, n_epochs_classifier=1, use_cuda=use_cuda)


def test_synthetic_2():
    synthetic_dataset = SyntheticDataset()
    run_benchmarks(synthetic_dataset, n_epochs=1, model=SVAEC, benchmark=True, use_cuda=use_cuda)


def test_cortex():
    cortex_dataset = CortexDataset()
    run_benchmarks(cortex_dataset, n_epochs=1, model=VAEC, use_cuda=use_cuda)


def test_brain_large():
    brain_large_dataset = BrainLargeDataset(subsample_size=128, save_path='tests/data/')
    run_benchmarks(brain_large_dataset, n_epochs=1, use_batches=False, tt_split=0.5, use_cuda=use_cuda)


def test_retina():
    retina_dataset = RetinaDataset(save_path='tests/data/')
    run_benchmarks(retina_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_cite_seq():
    pbmc_cite_seq_dataset = CiteSeqDataset(name='pbmc', save_path='tests/data/citeSeq/')
    run_benchmarks(pbmc_cite_seq_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_brain_small():
    brain_small_dataset = BrainSmallDataset(save_path='tests/data/')
    run_benchmarks(brain_small_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_hemato():
    hemato_dataset = HematoDataset(save_path='tests/data/HEMATO/')
    run_benchmarks(hemato_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)
...
def test_remote_loom():
    ...
    run_benchmarks(fish_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_cortex_loom():
    cortex_dataset = LoomDataset("Cortex.loom",
                                 save_path='tests/data/')
    run_benchmarks(cortex_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_anndata():
    ann_dataset = AnnDataset("test.h5ad", save_path='tests/data/')
    run_benchmarks(ann_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_csv():
    csv_dataset = CsvDataset("GSE100866_CBMC_8K_13AB_10X-RNA_umi.csv.gz", save_path='tests/data/', compression='gzip')
    run_benchmarks(csv_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)


def test_cbmc():
    cbmc_dataset = CbmcDataset(save_path='tests/data/citeSeq/')
    run_benchmarks(cbmc_dataset, n_epochs=1, show_batch_mixing=False, use_cuda=use_cuda)
