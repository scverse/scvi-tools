...
if is_package_installed("pyro"):
    ...
    class BetaBinomial(BetaBinomialDistribution):
        ...
        This is not allowed (``alpha`` and ``beta`` must be strictly greater than 0), so we clamp
        these values to be greater than a small constant ``eps``.
        """
        ...
        def __repr__(self) -> str:
            ...
            args_string = ", ".join(
                [
                    f"{p}: {param if param.numel() == 1 else param.size()}"
                    for p in param_names
                    if (param := self.__dict__[p]) is not None
                ]
            )
