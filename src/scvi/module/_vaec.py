...
class VAEC(EmbeddingModuleMixin, BaseMinifiedModeModuleClass):
    ...
    def __init__(
        ...
        if self.prior=='mog':
            ...
            self.prior_log_std = torch.nn.Parameter(
                    torch.zeros([n_labels, self.num_classes_mog, n_latent]) - 2.)
    ...
    def loss(
        ...
        if self.classifier is not None:
            ...
            classification_loss = classification_ratio * torch.masked_select(classification_loss_, mask).mean(0)
        
            loss = torch.mean(scaling_factor * (reconst_loss + classification_loss + kl_weight * kl_divergence_z))

            return LossOutput(
                loss=loss, reconstruction_loss=reconst_loss, kl_local=kl_divergence_z, classification_loss=classification_loss,
                logits=logits, true_labels=fine_labels
            )
        else:
