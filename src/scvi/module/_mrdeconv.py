...
import numpy as np
import torch
from torch.distributions import Categorical, Exponential, Independent, Laplace, MixtureSameFamily, Normal
from torch.distributions import kl_divergence as kl

from scvi import REGISTRY_KEYS
from scvi.distributions import NegativeBinomial
from scvi.module.base import EmbeddingModuleMixin, BaseModuleClass, LossOutput, auto_move_data
...
class MRDeconv(EmbeddingModuleMixin, BaseModuleClass):
    ...
    n_latent_amortization
        Number of dimensions used in the latent variables for the amortization encoder neural network
        ...
        if self.augmentation and self.training:
            with torch.no_grad():
                ...
                px_scale_augment_ = torch.nn.Softmax(dim=-1)(
                    self.px_decoder(self.decoder(decoder_input, enum_label.to(x.device))) + self.per_ct_bias[enum_label.ravel()].unsqueeze(-3) + self.beta.view(1, 1, -1))
                ...
                px_scale_augment = px_scale_augment.reshape(n_samples, m, self.n_labels, -1) # (samples, m, n_labels, n_genes)
                px_rate = library * px_scale_augment # (samples, m, n_labels, n_genes)
                ratios_ct_augmentation = torch.distributions.Dirichlet(
                    torch.zeros(self.n_labels) + 0.03).sample([n_samples, m]).to(x.device)
                ratios_ct_augmentation = ratios_ct_augmentation.reshape(n_samples, m, self.n_labels)
                # sum over celltypes
                augmentation_rate = torch.einsum('imc, imcg -> img', ratios_ct_augmentation, px_rate)  # (samples, m, n_genes)
                ratio_augmentation_ = torch.distributions.Beta(0.4, 0.5).sample([self.n_samples_augmentation-1, m]).unsqueeze(-1).to(x.device)
                ratio_augmentation = torch.cat([torch.zeros((n_samples_observed, m, 1), device=x.device), torch.ones((1, m, 1), device=x.device), ratio_augmentation_], dim=0)
    ...
    def generative(self, z, ind_x, library, batch_index):
        ...
        for mode in range(gamma_ind.shape[-4]):
            ...
            )  + self.per_ct_bias[enum_label.ravel()].reshape(1, m, self.n_labels, -1) + self.beta.view(1, 1, 1, -1)) # (n_samples, m, n_labels, n_genes)

        # add the additional cell types
        eps = eps.unsqueeze(0).repeat(n_samples, m, 1, 1) # (n_samples, m, add_celltypes, n_genes) <- this is the dummy cell type
        ...
        eps_v = self.eps_v if self.training else 0.
    ...
    def loss(
        ...
        if self.augmentation:
            prior_sampled = inference_outputs['prior_sampled'].reshape(n_samples, 1, x_augmented.shape[1], self.n_labels, self.n_latent)
            mean_vprior = torch.cat(
                [self.mean_vprior.unsqueeze(0).unsqueeze(-4).repeat(n_samples, m, 1, 1, 1),
                 prior_sampled.permute(0, 2, 3, 1, 4)],
                dim=-2)  # n_samples, m, n_labels, p + 1, n_latent
            var_vprior = torch.cat(
                [self.var_vprior.unsqueeze(0).unsqueeze(-4).repeat(n_samples, m, 1, 1, 1),
                 torch.min(self.var_vprior, dim=-2).values.view(
                     1, 1, self.n_labels, 1, self.n_latent).repeat(n_samples, m, 1, 1, 1)],
                dim=-2
        ...
        glo_neg_log_likelihood_prior = (
            -beta_reg * Normal(mean, scale).log_prob(self.beta).sum()
        )
        loss_augmentation = torch.tensor(0., device=x_augmented.device)
        if self.augmentation:
            expected_proportions = torch.cat([ratios_ct_augmentation, torch.zeros([n_samples, m, self.add_celltypes]).to(v.device)], dim=-1)
            loss_augmentation += weighting_augmentation * self._compute_cross_entropy(
                expected_proportions[sample_fully_augmented, ...].squeeze(0), v[sample_fully_augmented, ...].squeeze(0))
        if "x_smoothed" in tensors:
        ...
        v_sparsity_loss = ct_sparsity_weight * torch.distributions.Categorical(
            probs=v[0, ...]).entropy()
        v_sparsity_loss -= eta_reg * Exponential(
            torch.ones_like(v[0, :, -self.add_celltypes:])).log_prob(v[0, :, -self.add_celltypes:]).sum()
        
        loss = torch.mean(
            reconst_weight * reconst_loss + kl_weight * (neg_log_likelihood_prior + v_sparsity_loss) + glo_neg_log_likelihood_prior + loss_augmentation
        )
        ...
        return LossOutput(
            ...
            extra_metrics={"v_sparsity": v_sparsity_loss.mean(), "augmentation": loss_augmentation.mean()},
        )
