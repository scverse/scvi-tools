...
class MULTIVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        if self.use_observed_lib_size_atac:
            self.scaling_libsize = torch.nn.Parameter(torch.tensor(0.0))
            self.shift_libsize = torch.nn.Parameter(-torch.tensor(0.0))
        else:
        ...
        if self.atac_likelihood == "bernoulli":
            ...
            decoder_atac_kwargs = {"scale_activation": "softmax"}
        ...
        if self.use_observed_lib_size_atac:
            if self.atac_likelihood != "bernoulli":
                ...
                libsize_acc = torch.sigmoid(
                    libsize_acc / torch.exp(1000.0 * self.scaling_libsize)
                    - 1000.0 * self.shift_libsize
                )
        else:
    ...
    def loss(
        ...
        generative_outputs,
        kl_weight: float = 1.0,
        weighting_integration: float = 1.0,
    ):
        ...
        if assay_index is not None:
            ...
            kl_div_assays = torch.tensor(0.0, device=x.device)

        # KL WARMUP
        kl_local_for_warmup = kl_div_z
        weighted_kl_local = (
            kl_weight * kl_local_for_warmup + kl_div_paired + weighting_integration * kl_div_assays
        )
    ...
    def _compute_assay_penalty(
        ...
        if len(unique) > 1:
            for mask, params in zip(
                [mask1, mask2, mask3], [mod_param1, mod_param2, mod_param3], strict=True
            ):
