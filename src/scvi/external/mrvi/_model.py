...
class MRVI(JaxTrainingMixin, BaseModelClass):
        ...
        for array_dict in tqdm(scdl):
            ...
            if reqs.needs_sampled_distances or reqs.needs_normalized_distances:
                ...
                if reqs.needs_normalized_distances:
                    ...
                    normalized_dists = (
                        (sampled_dists - normalization_means)
    ...
    def differential_abundance(
        ...
        omit_original_sample
            Whether to omit the original sample from the differential abundance computation.
        ...
        * ``"{cov_key}_log_probs"``: For each key in ``sample_cov_keys``, an array of shape
        ...
        for sample_cov_key in sample_cov_keys:
            ...
            for sample_cov_value in sample_cov_unique_values:
                ...
                sel_log_probs = log_probs_arr.log_probs.loc[
                    {"sample": cov_samples}
                ].values
                if omit_original_sample:
                    sample_cov_one_hot = np.zeros((adata.n_obs, len(cov_samples)))
                    ...
                    ) - np.log((1 - sample_cov_one_hot).sum(axis=1))
                else:
                ...
                if compute_log_enrichment:
                    ...
                    if omit_original_sample:
                        rest_samples_one_hot = np.zeros(
                            (adata.n_obs, len(rest_samples))
                        )
                        for i, sample in enumerate(rest_samples):
                            rest_samples_one_hot[
                        ...
                        ) - np.log((1 - rest_samples_one_hot).sum(axis=1))
                    else:
            ...
            sample_cov_log_probs_map[sample_cov_key] = DataFrame.from_dict(
                per_val_log_probs
            )
    ...
    def differential_expression(
        ...
        * ``"n_samples"``: Number of admissible samples for each cell, if
        ...
        padj = false_discovery_control(pvalue.flatten(), method="bh").reshape(
            pvalue_shape
        )
    ...
    def _construct_design_matrix(
        ...
        if add_batch_specific_offsets:
            ...
            if cov.nunique() == self.summary_stats.n_batch:
                ...
                offset_indices = (
                    Series(np.arange(len(Xmat_names)), index=Xmat_names)
                    .loc[cov_names]
                    .values
                )
