        ...
        * ``"{cov_key}_log_probs"``: For each key in ``sample_cov_keys``, an array of shape
        ...
        for sample_cov_key in sample_cov_keys:
            ...
            for sample_cov_value in sample_cov_unique_values:
                ...
                sel_log_probs = log_probs_arr.log_probs.loc[{"sample": cov_samples}].values
                if omit_original_sample:
                    ...
                    val_log_probs = logsumexp(sel_log_probs_no_original, axis=1) - np.log(
                        (1 - sample_cov_one_hot).sum(axis=1)
                    )
                else:
                ...
                if compute_log_enrichment:
                    ...
                    if len(rest_samples) == 0:
                        ...
                        continue
                    rest_log_probs = log_probs_arr.log_probs.loc[{"sample": rest_samples}].values
                    if omit_original_sample:
                        rest_samples_one_hot = np.zeros((adata.n_obs, len(rest_samples)))
                        for i, sample in enumerate(rest_samples):
                            rest_samples_one_hot[adata.obs[self.sample_key] == sample, i] = 1
