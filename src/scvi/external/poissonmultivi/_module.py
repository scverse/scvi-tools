...
from scvi.module._utils import masked_softmax
...
class POISSONMULTIVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        self.use_size_factor_key_expr = use_size_factor_key_expr
        self.use_size_factor_key_acc = use_size_factor_key_acc
        ...
        self.z_decoder_expression = DecoderSCVI(
            ...
            scale_activation="softplus" if use_size_factor_key_expr else "softmax",
        )

        # accessibility
        # accessibility dispersion parameters
        self.px_r_acc = torch.nn.Parameter(torch.randn(n_input_regions))
        ...
        self.z_decoder_accessibility = DecoderSCVI(
            ...
            use_batch_norm=self.use_batch_norm_decoder,
            use_layer_norm=self.use_layer_norm_decoder,
            scale_activation="softplus" if use_size_factor_key_acc else "softmax",
        )
    ...
    def _get_generative_input(self, tensors, inference_outputs, transform_batch=None):
        ...
        size_factor_key_expr = REGISTRY_KEYS.SIZE_FACTOR_KEY_EXPR
        size_factor_expr = (
            torch.log(tensors[size_factor_key_expr])
            if size_factor_key_expr in tensors.keys()
            else None
        )
        size_factor_key_acc = REGISTRY_KEYS.SIZE_FACTOR_KEY_ACC
        size_factor_acc = (
            torch.log(tensors[size_factor_key_acc])
            if size_factor_key_acc in tensors.keys()
            else None
        )
        ...
        input_dict = {
            ...
            "libsize_expr": libsize_expr,
            "libsize_acc": libsize_acc,
            "size_factor_expr": size_factor_expr,
            "size_factor_acc": size_factor_acc,
            "label": label,
        }
        return input_dict
    ...
    def generative(
        ...
        libsize_expr=None,
        libsize_acc=None,
        size_factor_expr=None,
        size_factor_acc=None,
        use_z_mean=False,
        ...
        if not self.use_size_factor_key_acc:
            size_factor_acc = libsize_acc
        px_scale_acc, _, px_rate_acc, px_dropout_acc = self.z_decoder_accessibility(
            "gene",
            decoder_input,
            size_factor_acc,
            batch_index,
        ...
        px_r_acc = self.px_r_acc
        px_r_acc = torch.exp(px_r_acc)

        # Expression Decoder
        if not self.use_size_factor_key_expr:
            size_factor_expr = libsize_expr
        ...
        return {
            "px_scale_acc": px_scale_acc,
            "px_r_acc": px_r_acc,
            "px_rate_acc": px_rate_acc,
            "px_dropout_acc": px_dropout_acc,
            "px_scale": px_scale,
            "px_r": px_r,
            "px_rate": px_rate,
    ...
    def loss(self, tensors, inference_outputs, generative_outputs, kl_weight: float = 1.0):
        ...
        px_rate_acc = generative_outputs["px_rate_acc"]
        px_r_acc = generative_outputs["px_r_acc"]
        px_dropout_acc = generative_outputs["px_dropout_acc"]
        rl_accessibility = self.get_reconstruction_loss_accessibility(
            x_chr, px_rate_acc, px_r_acc, px_dropout_acc
        )
        # Compute Expression loss
        px_rate = generative_outputs["px_rate"]
        px_r = generative_outputs["px_r"]
        px_dropout = generative_outputs["px_dropout"]
        rl_expression = self.get_reconstruction_loss_expression(x_rna, px_rate, px_r, px_dropout)
    ...
    def get_reconstruction_loss_accessibility(self, x, px_rate, px_r, px_dropout):
