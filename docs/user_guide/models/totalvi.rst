...
Next, we model each cell's protein and RNA expression to be generated by the following process:
...
.. list-table::
   ...
   * - :math:`\rho_n \in \Delta^{G-1}`
     - Denoised/normalized gene expression, ``px_["scale"]`` in the code.
   * - :math:`\alpha_n \in [1, \infty)^T`
     - Foreground scaling factor for proteins, identifies the mixture distribution, ``py_["rate_fore"]`` in the code.
   * - :math:`\pi_n \in (0, 1)^T`
     - Probability of background for each protein, ``py_["mixing"]`` in the code (logits scale).
   * - :math:`l_n \in (0, \infty)`
...
Integrating out :math:`v_{nt}` yields a negative binomial conditional distribution for :math:`y_{nt}`.
