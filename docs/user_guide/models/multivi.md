...
MultiVI takes as input a multiome single-cell matrix $X\_{mult}$ with $N$ cells and $G$ genes and
$M$ genomic regions (peaks), or just a scRNA-seq gene expression matrix $X\_{rna}$ with $N$ cells and
$G$ genes or just a scATAC-seq accessibility matrix $X\_{acc}$ with $N$ cells and $M$ genomic
...
MultiVI posits that the observed UMI counts for cell $n$, gene $g$, $x\_{ng}$, and regions $j$,
$y\_{nj}$, are generated by the following process:
...
For RNA-seq, the prior parameters $\\ell\_\\mu$ and $\\ell\_{\\sigma^2}$ are computed per batch as the mean and
...
For ATAC-seq, detecting a region as accessible ($y\_{nj} > 0$) is
