...
Gene Expression Comparisons: SCVI allows you to perform differential expression analysis between different clusters or conditions, such as comparing cell types, disease states, treatments, or experimental groups.

A common use case is to compare two conditions within each cell type. This can be done by looping over cell types and using the `idx1` and `idx2` arguments of {meth}`~scvi.model.SCVI.differential_expression` to define the cells in each comparison.
...
for cell_type in adata.obs["cell_type"].unique():
    idx1 = (adata.obs["cell_type"] == cell_type) & (adata.obs["condition"] == "treated")
    idx2 = (adata.obs["cell_type"] == cell_type) & (adata.obs["condition"] == "control")

    cell_type_de = model.differential_expression(
        idx1=idx1,
        idx2=idx2,
    )
    cell_type_de["cell_type"] = cell_type
    de_results.append(cell_type_de)
...
6. Predictive Modeling and Imputation
...
9. Custom Annotations and Metadata
...
10. Evaluation of Model Performance
