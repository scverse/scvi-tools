...
- Add support for padding layers in {method}`~scvi.model.SCVI.prepare_query_anndata` which is necessary to run {method}`~scvi.model.SCVI.load_query_data` for a model setup with a layer instead of X [#1575].
