...
- `backup_url` argument added to {meth}`~scvi.model.base.BaseModelClass.load` ([#1504]).

## Breaking changes
- Support for loading legacy loading is removed from {meth}`~scvi.model.base.BaseModelClass.load`. Utility to convert old files to the new file as been added {meth}`{~scvi.model.base.BaseModelClass.convert_legacy_save` ([#1504]).
