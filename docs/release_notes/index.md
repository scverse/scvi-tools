...
-   Add experimental support for modeling batch effects with an embedding layer
    in {class}`scvi.model.SCVI` with argument `use_batch_embedding` and
    {class}`scvi.model.base.VAEMixin.get_batch_representation` {pr}`2240`.
