...
class PosteriorPredictiveCheck:
    ...
    def __init__(self, n_samples: int = 50, raw_counts: np.ndarray = None):
        ...
        """
        self.raw_counts = (
            sparse.COO(raw_counts) if type(raw_counts) is np.ndarray else None
        )
    ...
    def store_fa_samples(
        ...
        temp_counts = self.raw_counts.todense()
        # reconstruction
        if normalization == "log":
            data = np.log(temp_counts + 1)
            key += " (Log)"
        elif normalization == "rate":
            lib_size_rna = temp_counts[:, : self.dataset.nb_genes].sum(axis=1)[
                :, np.newaxis
            ]

            data = np.log(
                10000 * temp_counts[:, : self.dataset.nb_genes] / lib_size_rna + 1
            )
            key += " (Rate)"
    ...
    def store_pca_samples(self, key="PCA", normalization="log", **kwargs):
        ...
        z_mean = np.matmul(
            np.matmul(linalg.inv(M), pca.components_),
            (self.raw_counts.todense() - pca.mean_).T,
        ).T
    ...
    def store_external_samples(self, samples: sparse.COO, key: str):
        ...
        if (
            samples.shape[2] != self.n_samples
            and samples.shape[:2] != self.raw_counts.shape
        ):
            raise ValueError("Dimension should be cells by genes by samples")

        self.posterior_predictive_samples[key] = sparse.COO(samples)
    ...
    def mean_squared_log_error(self):
        ...
        for m, samples in self.posterior_predictive_samples.items():
            ...
            msle_gene = np.mean(
                np.square(
                    np.log(point_sample.todense()[:, : self.dataset.nb_genes] + 1)
                    - np.log(self.raw_counts.todense()[:, : self.dataset.nb_genes] + 1)
                )
            )
            df[m] = msle_gene
