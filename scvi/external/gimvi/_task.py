...
class GIMVITask(AdversarialTask):
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        if optimizer_idx == 0:
            ...
            for (i, tensors) in enumerate(batch):
                ...
                loss_kwargs = dict(kl_weight=self.kl_weight, mode=i)
            ...
            batch_tensor = [
                torch.zeros((z.shape[0]), device=z.device) + i for i, z in enumerate(zs)
            ]
        ...
        if optimizer_idx == 1:
            ...
            loss = self.loss_adversarial_classifier(
                torch.cat(zs).detach(), batch_tensor, True
            )
            loss *= kappa
    ...
    def validation_step(self, batch, batch_idx):
        inference_kwargs = dict(mode=batch_idx)
        _, _, scvi_loss = self.forward(
            batch, loss_kwargs=self.loss_kwargs, inference_kwargs=inference_kwargs
        )
