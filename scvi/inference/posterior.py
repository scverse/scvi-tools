...
class Posterior:
    def __init__(self, model, gene_dataset, data_loader, name='', to_monitor=[]):
        self.model = model
    ...
    def ll(self, verbose=False):
        ll = compute_log_likelihood(self.model, self.data_loader)
        if verbose:
            print("LL %s is : %.4f" % (self.name, ll))
        return ll

    ll.mode = 'min'

    def marginal_ll(self, verbose=False, n_mc_samples=1000):
        ll = compute_marginal_log_likelihood(self.model, self.data_loader, n_mc_samples)
        if verbose:
            print("True LL for %s is : %.4f" % (self.name, ll))
        return ll
        ...
        if self.gene_dataset.n_labels > 1:
            ...
            if verbose:
                print("Clustering Scores for %s:\nSilhouette: %.4f\nNMI: %.4f\nARI: %.4f\nUCA: %.4f" %
                      (self.name, asw_score, nmi_score, ari_score, uca_score))
            return asw_score, nmi_score, ari_score, uca_score

    def nn_overlap_score(self, verbose=True, **kwargs):
        if hasattr(self.gene_dataset, 'adt_expression_clr'):
            assert self.name == 'sequential'  # only works for the sequential data_loader (mapping indices)
            latent, _, _ = self.get_latent()
            ...
            if verbose:
                print("Overlap Scores for %s:\nSpearman Correlation: %.4f\nFold Enrichment: %.4f" %
                      (self.name, spearman_correlation, fold_enrichment))
            return spearman_correlation, fold_enrichment

    def differential_expression(self, *args, verbose=False, **kwargs):
        px_scale, all_labels = self.de_stats(*args, **kwargs)
        ...
        if type(self.gene_dataset) == CortexDataset:
            ...
            if verbose:
                print("DE score for cortex on %s is : %.4f" % (self.name, de_score))
            return de_score
    ...
    def get_latent(self):
        ...
        batch_indices = []
        labels = []
        for tensors in self.data_loader:
    ...
    def de_stats(self, M_sampling=100):
        """
    ...
    def raw_data(*posteriors):
        ...
        return [get_data_labels(posterior.data_loader) for posterior in posteriors]


class PosteriorFish(Posterior):
    def ll(self, verbose=False):
        ll = compute_log_likelihood(self.model, self.data_loader, mode="smFISH")
        if verbose:
            print("LL for %s is : %.4f" % (self.name, ll))
        return ll
