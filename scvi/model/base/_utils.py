    ...
    if isinstance(adata, mudata.MuData):
        for modality in source_var_names.keys():
            ...
            if not np.array_equal(source_var_names[modality], user_var_names):
                warnings.warn(
                    f"var_names for {modality} in mudata passed in does not match var_names"
                    f"of mudata used to train the model. For valid results, the vars need to be"
                    f"the same and in the same order as the mdata used to train the model.",
                    UserWarning,
