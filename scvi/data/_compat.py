        ...
        if attr_name == _constants._ADATA_ATTRS.X:
            ...
            if new_registry_key in {REGISTRY_KEYS.BATCH_KEY, REGISTRY_KEYS.LABELS_KEY}:
                ...
                if model_cls.__name__ == "TOTALVI" and new_registry_key == REGISTRY_KEYS.LABELS_KEY:
                    continue
                setup_args[f"{new_registry_key}_key"] = categorical_mappings[attr_key]["original_key"]
...
def registry_from_setup_dict(model_cls, setup_dict: dict, unlabeled_category: Optional[str] = None) -> dict:
    ...
    for (
        ...
        if attr_name in (_constants._ADATA_ATTRS.X, _constants._ADATA_ATTRS.LAYERS):
            ...
            if model_cls.__name__ == "TOTALVI" and new_registry_key == REGISTRY_KEYS.LABELS_KEY:
                field_state_registry[CategoricalObsField.CATEGORICAL_MAPPING_KEY] = np.zeros(1, dtype=np.int64)
            else:
                field_state_registry[CategoricalObsField.CATEGORICAL_MAPPING_KEY] = categorical_mapping["mapping"]
            field_state_registry[CategoricalObsField.ORIGINAL_ATTR_KEY] = categorical_mapping["original_key"]
            if new_registry_key == REGISTRY_KEYS.BATCH_KEY:
                ...
                if unlabeled_category is not None:
                    field_state_registry[LabelsWithUnlabeledObsField.UNLABELED_CATEGORY] = unlabeled_category
        elif attr_name == _constants._ADATA_ATTRS.OBSM:
            if new_registry_key == REGISTRY_KEYS.CONT_COVS_KEY:
                ...
                field_state_registry[CategoricalJointObsField.FIELD_KEYS_KEY] = field_state_registry.pop("keys")
                field_summary_stats[f"n_{new_registry_key}"] = len(extra_categoricals_mapping["keys"])
            elif new_registry_key == REGISTRY_KEYS.PROTEIN_EXP_KEY:
                field_state_registry[ProteinObsmField.COLUMN_NAMES_KEY] = setup_dict["protein_names"].copy()
                if "totalvi_batch_mask" in setup_dict:
                    field_state_registry[ProteinObsmField.PROTEIN_BATCH_MASK] = setup_dict["totalvi_batch_mask"].copy()
